...
#     }
#
#     for i in Rew_functions:
#         reward_funcs = [i[0], i[1]]
#         print('no. of items :- '+str(n_items))
#         print('reward function other than likelihood :-'+i[1])
#
#         envs = [
#           ('EFC', EFCEnv),
#           ('HLR', HLREnv),
...
#         }
#
#         reward_file = str(n_items)+i[1]+'_reward_logs.pkl'
#         with open(os.path.join(data_dir, reward_file), 'wb') as f:
#           pickle.dump(reward_logs, f, pickle.HIGHEST_PROTOCOL)
#
#         R = np.zeros((6, len(tutor_builders), n_eps, n_reps))
#         for i in range(6):
...
#
#                 plt.legend(loc='upper left')
#                 plt.savefig(os.path.join(data_dir, '%s_%s_%s-%s.pdf' % (str(n_items), i[1], env_name, reward_func)), bbox_inches='tight')
#                 plt.show()
